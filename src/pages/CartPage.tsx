              ...
              <div key={product._id} className="flex flex-col sm:flex-row gap-3 sm:gap-4 bg-card border border-border rounded-xl p-3 sm:p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 sm:mt-2">
                    ...
                    {product.mrp > product.price && <span className="text-xs sm:text-sm text-muted-foreground line-through">₹{formatPrice(product.mrp)}</span>}
                  </div>
                </div>
                <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-2 pt-2 sm:pt-0 border-t sm:border-t-0 border-border">
                  <div className="flex items-center border border-border rounded-lg">
                    ...
                    <span className="px-3 sm:px-3 text-sm min-w-[2rem] text-center">{quantity}</span>
                    <button type="button" onClick={() => updateQuantity(product._id, quantity + 1)} className="p-3 sm:p-2 hover:bg-secondary active:bg-secondary/80 touch-manipulation min-h-[44px] min-w-[44px] flex items-center justify-center"><Plus className="w-4 h-4" /></button>
                  </div>
                  <span className="font-bold text-sm sm:text-base">₹{formatPrice(product.price * quantity)}</span>
                  <button type="button" onClick={() => removeFromCart(product._id)} className="text-muted-foreground hover:text-destructive p-3 min-h-[44px] min-w-[44px] flex items-center justify-center touch-manipulation rounded-lg"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            ))}
          </div>
